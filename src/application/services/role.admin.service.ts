...
export class RoleAdminService implements IRoleAdminService {
    ...
    async createRole(adminUser: AdminUser, details: { roleName: string; description?: string }): Promise<Role> {
        ...
        // const existing = await this.roleRepository.findByName(adminUser.tenantId, details.roleName);
        // if (existing) {
        //     throw new RoleExistsError(details.roleName);
        // }

        const newRole = new Role(adminUser.tenantId, details.roleName, details.description); // Create domain entity instance
        try {
    ...
    async listRoles(adminUser: AdminUser, options?: QueryOptions): Promise<QueryResult<Role>> {
        ...
        try {
            const result = await this.roleRepository.list(adminUser.tenantId, options);
    ...
    async updateRole(adminUser: AdminUser, roleName: string, updates: { description?: string }): Promise<Role | null> {
        ...
        try {
            const updatedRole = await this.roleRepository.update(adminUser.tenantId, roleName, updates);
            if (!updatedRole) {
    ...
    async deleteRole(adminUser: AdminUser, roleName: string): Promise<void> {
        ...
        let deleted: boolean;
        try {
            deleted = await this.roleRepository.delete(adminUser.tenantId, roleName);
        } catch (error: any) {
        ...
        try {
            await this.assignmentRepository.removeAllAssignmentsForRole(adminUser.tenantId, roleName);
    ...
    async assignPermissionToRole(adminUser: AdminUser, roleName: string, permissionName: string): Promise<void> {
        ...
        try {
            await this.assignmentRepository.assignPermissionToRole(adminUser.tenantId, roleName, permissionName);
    ...
    async removePermissionFromRole(adminUser: AdminUser, roleName: string, permissionName: string): Promise<void> {
        ...
        try {
            await this.assignmentRepository.removePermissionFromRole(adminUser.tenantId, roleName, permissionName);
    ...
    async listPermissionsForRole(adminUser: AdminUser, roleName: string): Promise<string[]> {
        ...
        try {
            const permissionNames = await this.assignmentRepository.findPermissionsByRoleName(adminUser.tenantId, roleName);
