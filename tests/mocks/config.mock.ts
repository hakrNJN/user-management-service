...
const mockConfigService: jest.Mocked<IConfigService> = {
    ...
    getBoolean: jest.fn((key: string, defaultValue?: boolean): boolean | undefined => {
        ...
        if (processedValue === 'true' || processedValue === '1') return true;
        if (processedValue === 'false' || processedValue === '0') return false;
    ...
    getNumberOrThrow: jest.fn((key: string): number => {
        // Use the mock 'get' implementation (without default)
    ...
    getBooleanOrThrow: jest.fn((key: string): boolean => {
        // Use the mock 'get' implementation (without default)
        const value = mockConfigService.get(key, undefined);
        if (value === undefined || value === '') {
             throw new Error(`Configuration error: Required environment variable "${key}" is missing or empty.`);
